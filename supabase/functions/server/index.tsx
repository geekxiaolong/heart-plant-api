...
const envServiceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")?.trim();
const supabaseServiceRoleKey = envServiceRoleKey || defaultAnonKey;
const hasServiceRoleKey = Boolean(envServiceRoleKey);
...
if (!hasServiceRoleKey) {
  console.warn(
    "[config] SUPABASE_SERVICE_ROLE_KEY is missing. Read-only routes may still work, but writes/admin storage setup can fail due to RLS.",
  );
}
...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    desc: "拥有 3 棵及以上植物",
    icon: "🌿",
    requirement: { type: "plants", count: 3 },
    reward: 150,
  },
  {
    id: "a3",
    ...
    desc: "累计进行 5 次浇水",
    icon: "💧",
    requirement: { type: "water", count: 5 },
    reward: 100,
  },
  {
    id: "a4",
  ...
  },
  {
    ...
    desc: "获得 10 个点赞",
    icon: "✨",
    requirement: { type: "likes", count: 10 },
    reward: 150,
  },
  {
    id: "a6",
  ...
  },
  {
    ...
    desc: "获得 50 个点赞",
    icon: "👑",
    requirement: { type: "likes", count: 50 },
    reward: 500,
  },
  {
    id: "a8",
    ...
    desc: "累计进行 20 次浇水",
    icon: "🌊",
    requirement: { type: "water", count: 20 },
    reward: 400,
  },
];
    ...
    for (const achievement of ACHIEVEMENTS) {
      ...
      switch (reqType) {
        ...
        case "comments":
          ...
          break;
        case "water":
          currentValue = stats.waterCount;
          break;
        case "fertilizer":
          currentValue = stats.fertilizerCount;
          break;
        case "plants":
          currentValue = stats.plantsAdopted;
          break;
        case "streak":
...
const registerRoutes = (r: Hono) => {
  ...
  r.post("/signup", async (c) => {
    try {
      ...
      const { data, error } = await supabase.auth.admin.createUser({
        ...
        user_metadata: {
          name,
          role: email.toLowerCase() === "776427024@qq.com" ? "admin" : "user",
        },
        email_confirm: true,
      });
  ...
  r.post("/seed-batch", async (c) => {
    try {
      ...
      for (const item of items) {
        ...
        let key = "";
        if (type === "mood") {
          key = `mood:${data.plantId}:${Date.now()}_${
            Math.random().toString(36).substring(2, 6)
          }`;
        } else if (type === "journal") {
          key = `journal:${data.plantId}:${Date.now()}_${
            Math.random().toString(36).substring(2, 6)
          }`;
        } else if (type === "log-activity") {
          key = `log:${data.plantId}:${Date.now()}_${
            Math.random().toString(36).substring(2, 6)
          }`;
        } else {
          key = `${type}:${Date.now()}_${
  ...
  r.post("/log-activity", async (c) => {
    try {
      ...
      return c.json(
        { error: "Failed to log activity", details: err.message },
        400,
      );
    }
  });
...
const initBucket = async () => {
  if (!hasServiceRoleKey) {
    console.warn(
      "[startup] Skipping bucket initialization because SUPABASE_SERVICE_ROLE_KEY is not configured.",
    );
    return;
  }
...
const initializeMockPlants = async () => {
  if (!hasServiceRoleKey) {
    console.warn(
      "[startup] Skipping mock library initialization because SUPABASE_SERVICE_ROLE_KEY is not configured.",
    );
    return;
  }
    ...
    const mockPlants = [
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1677833524795-a815afaff22f?q=80&w=1080",
        tags: ["净化空气", "耐阴"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1648070024741-43f8fa965966?q=80&w=1080",
        tags: ["垂吊", "多肉"],
      ...
      },
      {
        ...
        scene: "solo",
        description:
          "巨大的叶片如同龟背，极具热带风情，是非常受欢迎的室内植物。",
        imageUrl:
          "https://images.unsplash.com/photo-1713755008116-773bfc0a6431?q=80&w=1080",
        tags: ["北欧风", "耐阴"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1596547612397-1432a7a7d37d?q=80&w=1080",
        tags: ["网红植物", "大中型"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1593482892290-f54927ae1bb6?q=80&w=1080",
        tags: ["强力净化", "懒人植物"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1674482918961-57a3a0484bf3?q=80&w=1080",
        tags: ["新手推荐", "水培土培"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1658545056023-ce30117cd9ad?q=80&w=1080",
        tags: ["助眠", "紫色浪漫"],
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1761371290431-d5b23356eaa7?q=80&w=1080",
        tags: ["精致", "多肉"],
